...
set -x
set -euo pipefail  # -e: 命令失败退出 -u: 未定义变量报错 -o pipefail: 管道命令失败退出
trap 'echo "Error at line $LINENO"; exit 1' ERR  # 捕获ERR信号并打印出错行号[1,4](@ref)
...
replacement=True
PREFIX=${project_name}-${model}-${algorithm}-${policy_loss}-${data}-DAPO_${enable_filter_groups}_replacement_${replacement}-${num_rollout_min}-${num_rollout_max}-${entropy_param}

MODEL_ROOT=/root/paddlejob/workspace/env_run/output/models
DATA_ROOT=/root/paddlejob/workspace/env_run/output/YOUR_DATA_ROOT
...
for i in {1..15}; do
    ...
    INTER_DIR=/root/paddlejob/workspace/env_run/output/CurES/epic/Intermediate/${PREFIX}/iter_${i}
    if [ $i -eq 1 ]; then
    ...
    bash ./epic/run_initialize.sh $model_name_or_path $i $num_rollout_min $num_rollout_max $correct_threshold $entropy_param $train_batch_size $INTER_DIR
    wait

    numina=$DATA_ROOT/GVM/data/numina_math_${i}/train.parquet
    train_files="['$numina']"
    ...
    CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python3 -m epic.main_epic \
        ...
        +data.epic_replacement=${replacement} \
